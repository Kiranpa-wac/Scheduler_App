...
const EditEventModal = ({ open, onClose, event, onUpdate, existingEvents = [] }) => {
  const [title, setTitle] = useState(event?.title || "");
  const [organiser, setOrganiser] = useState(event?.organiser || "");
  const [project, setProject] = useState(event?.project || "");
  const [startDate, setStartDate] = useState(
    event ? new Date(event.start).toISOString().slice(0, 10) : ""
  );
  const [startTime, setStartTime] = useState(
    event ? new Date(event.start).toTimeString().slice(0, 5) : ""
  );
  const [endDate, setEndDate] = useState(
    event ? new Date(event.end).toISOString().slice(0, 10) : ""
  );
  const [endTime, setEndTime] = useState(
    event ? new Date(event.end).toTimeString().slice(0, 5) : ""
  );
  ...
  useEffect(() => {
    if (event) {
      ...
      setEndDate(new Date(event.end).toISOString().slice(0, 10));
      ...
      <DialogContent className="sm:max-w-md md:max-w-lg bg-white p-0 rounded-xl shadow-xl border-0">
        <DialogHeader>
          <VisuallyHidden>
            ...
            <DialogDescription>Modal for editing an event</DialogDescription>
          </VisuallyHidden>
        </DialogHeader>
        <Card className="border-0 shadow-none">
          <CardHeader className="bg-blue-50 rounded-t-xl pb-4 pt-6">
            <CardTitle className="text-2xl font-bold text-blue-800">Edit Event</CardTitle>
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-5 pt-6">
              ...
              <div className="flex flex-col space-y-2">
                ...
                {errors.organiser && (
                  <span className="text-red-500 text-xs">{errors.organiser}</span>
                )}
              </div>
              {/* Project */}
              <div className="flex flex-col space-y-2">
                ...
                {errors.project && (
                  <span className="text-red-500 text-xs">{errors.project}</span>
                )}
              </div>
              {/* Date & Time */}
              <div className="pt-2 pb-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="relative">
                    ...
                    />
                    {errors.startTime && (
                      <span className="text-red-500 text-xs">{errors.startTime}</span>
                    )}
                  </div>
                  {/* End Date */}
                  <div className="relative">
                  ...
                  <div className="relative">
                    ...
                    />
                    {errors.endTime && (
                      <span className="text-red-500 text-xs">{errors.endTime}</span>
                    )}
                  </div>
                </div>
              </div>
